...
public class SolicitacaoCaronaService {
    ...
    public void cancelarSolicitacao(final Long id) {
        ...
        if (solicitacao.getEstudante() != currentUserService.getCurrentEstudante()) {
            throw new ErroDePermissao(MensagensResposta.SOLICITACAO_CARONA_NAO_PERTENCE_ESTUDANTE);
        }

        if (solicitacao.getStatus() == StatusSolicitacaoCarona.PEDIDO_DE_ENTRADA) {
            throw new ErroDeCliente(MensagensResposta.SOLICITACAO_CARONA_JA_VIROU_PEDIDO_ENTRADA);
        }

        solicitacao.setStatus(StatusSolicitacaoCarona.CANCELADO);
        solicitacaoRepository.save(solicitacao);
        log.info("Solicitação ID: {} cancelada com sucesso", id);
    }

    @Transactional
    public void cancelarSolicitacoesDeCaronaDoEstudante(final Estudante estudante) {
